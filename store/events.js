...
export const mutations = {
  addEvent(state, event) {
    ...
    if (len === 0 || event.eventType !== 'PRODUCT_VIEW') {
      state.log.push(event)
    } else if (state.log[len - 1].eventType !== event.eventType) {
      state.log.push(event)
    }
  }
}
...
export const actions = {
  ...
  productReorder({ commit, rootState }) {
    commit('addEvent', {
      eventType: 'PRODUCT_REORDER',
      ...eventProperties(rootState)
    })
  },

  pauseSubscription({ commit, rootState }) {
    commit('addEvent', {
      eventType: 'PAUSE_SUBSCRIPTION',
      ...eventProperties(rootState)
    })
  }
}
